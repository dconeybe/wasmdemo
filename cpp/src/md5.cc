...
void CalculateMd5Digest(const uint8_t* data, int32_t len, uint8_t dest[kDigestLength]) {
  ...
  const auto size = static_cast<decltype(std::string_view().size())>(len);

  MD5Context ctx;
  MD5Init(&ctx);
  MD5Update(&ctx, data, size);
  static_assert(sizeof(MD5Digest) == sizeof(uint8_t[16]));
  MD5Final(reinterpret_cast<MD5Digest*>(dest), &ctx);
}
